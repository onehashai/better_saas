...
def setup(account_request):
    try:
        ...
        if stock_list:
            ...
            stock_site_doc = frappe.get_doc(
                "Stock Sites", stock_site, ignore_permissions=True
            )
            ...
            commands = [
                "bench new-site --mariadb-root-password {mysql_password} --admin-password {admin_password} {site_name}".format(
                    site_name=site_name,
            ...
            if saas_settings.install_erpnext:
                ...
                commands.append(
                    "bench --site {site_name} install-app erpnext journeys".format(
                        site_name=site_name
                    )
                )
            else:
        ...
        if saas_user.domain_type == "Private":
            ...
            commands.append(
                "bench setup add-domain {custom_domain} --site {site_name}".format(
                    custom_domain=custom_domain, site_name=site_name
                )
            )
        ...
        commands.append(
            "bench --site "
            + master_site_name
            + " execute better_saas.better_saas.doctype.saas_user.saas_user.create_first_user_on_target_site --args="
            + '"'
            + "['{saas_user}']".format(
                master_site_name=master_site_name, saas_user=saas_user.name
            )
            + '"'
        )
        (
            ...
            discounted_users,
            customer,
        ) = get_site_limits(saas_user.promocode, saas_settings)
        commands.append(
            "bench --site {site_name} set-limits --limit users {limit_users} --limit emails {limit_emails} --limit space {limit_space} --limit email_group {limit_email_group} --limit expiry {limit_expiry}".format(
                site_name=site_name,
        ...
        saas_site.base_plan = (
            saas_settings.base_plan_india
            if saas_user.country == "India"
            else saas_settings.base_plan_international
        )
        ...
        if saas_user.promocode:
            promocode = frappe.get_doc(
                "Coupon Code",
                {"coupon_code": saas_user.promocode},
                ignore_permissions=True,
            )
...
def get_site_limits(promocode, saas_settings):
    coupon_code = (
        frappe.get_doc("Coupon Code", {"coupon_code": promocode}) if promocode else None
    )
    discounted_users = (
        coupon_code.discounted_users
        if coupon_code and coupon_code.discounted_users
        else 0
    )
    limit_emails = (
        int(coupon_code.limit_for_emails)
        if coupon_code and int(coupon_code.limit_for_emails)
        else int(saas_settings.default_limit_for_emails)
    )
    limit_space = (
        int(coupon_code.limit_for_space)
        if coupon_code and int(coupon_code.limit_for_space)
        else int(saas_settings.default_limit_for_space)
    )
    limit_email_group = (
        int(coupon_code.limit_for_email_group)
        if coupon_code and int(coupon_code.limit_for_email_group)
        else int(saas_settings.default_limit_for_email_group)
    )
    limit_users = (
        0
        if coupon_code and coupon_code.unlimited_users
        else (
            int(coupon_code.limit_for_users)
            if coupon_code and coupon_code.limit_for_users
            else int(saas_settings.default_limit_for_users)
        )
    )  ## Applying Users count from promocode
    ...
    if coupon_code and coupon_code.no_expiry == 1:
        ...
        limit_expiry = (
            add_days(today(), int(coupon_code.expiry))
            if coupon_code and coupon_code.expiry > 0
            else add_days(today(), int(saas_settings.default_expiry))
        )
...
def get_status(account_request):
    ...
    if doc.key:
        ...
        frappe.throw(
            "You will get confirmation email once your site is ready.",
            "ValidationError",
        )
    ...
    if commandStatus.get("status") in ["", "Ongoing"]:
        ...
        stock_site = frappe.get_value(
            "Stock Sites", filters={"renamed": doc.linked_saas_site, "status": "Picked"}
        )
        if stock_site:
...
def delete_site(site_name):
    try:
        ...
        integ_req = frappe.get_list(
            "Integration Request", {"reference_docname": site_name}
        )
        if integ_req:
        ...
        finrich_list = frappe.get_list(
            "FinRich Archive", filters={"reference_site": site.name}
        )
        if finrich_list:
        ...
        user.delete()
        site_deletion_config = frappe.get_doc(
            "Site Deletion Configuration", "Site Deletion Configuration"
        )
        ...
        if template:
            ...
            if email_template:
                ...
                frappe.sendmail(
                    user.email, subject=email_template.subject, message=message
                )

        commands = [
            "bench drop-site {site_name} --root-password {mysql_password}".format(
                site_name=site_name, mysql_password=mysql_password
            )
        ]
        commands.append("bench setup nginx --yes")
        commands.append("bench setup reload-nginx")
...
def check_subdomain_avai(subdomain):
    if not bool(re.match("^[a-zA-Z0-9]+$", subdomain)):
        frappe.throw(
            "Sub-domain can only contain letters and numbers", "ValidationError"
        )
    ...
    if frappe.db.exists("Saas Domains", {"domain": subdomain}):
        ...
        if (
            saas_settings.include_management_site_subdomain
            and saas_settings.management_subdomain == subdomain
        ):
            status = "False"
        else:
            status = "True"
...
def apply_new_limits(
    ...
    commands.append(
        "bench --site {site_name} set-config mute_emails {mute_emails}".format(
            site_name=site_name, mute_emails=mute_email_flag
        )
    )
    frappe.enqueue(
        "bench_manager.bench_manager.utils.run_command",
...
def signup(
    ...
    country = (
        country if country else (geo_country["names"]["en"] if geo_country else None)
    )
    password_test_result = check_password_strength(
        passphrase, first_name, last_name, email
    )
    if not password_test_result["feedback"]["password_policy_validation_passed"]:
        frappe.throw(
            password_test_result["feedback"]["warning"]
            + "\r\n"
            + password_test_result["feedback"]["suggestions"][0],
            "ValidationError",
        )

    existing_saas_user = frappe.get_list(
        "Saas User", filters={"email": email, "linked_saas_site": ""}
    )
...
def create_lead(saas_user):
    ...
    if saas_user.promocode:
        customer = frappe.get_value(
            "Coupon Code", {"coupon_code": saas_user.promocode}, "customer"
        )
    ...
    if existing_lead:
        ...
        if (
            ...
            and lead_doc.contact_date.strftime("%Y-%m-%d %H:%M:%S.%f")
            < frappe.utils.now()
        ):
            lead_doc.contact_date = ""
...
@frappe.whitelist(allow_guest=True)
def update_other_details_request(
    id, company=None, users=None, designation=None, referral_source=None
):
    doc = frappe.get_doc("Saas User", id)
    try:
...
def apply_promocode(promocode, site_name):
    # return True
    saas_user = frappe.get_list(
        "Saas User", filters={"linked_saas_site": site_name}, ignore_permissions=True
    )
    if len(saas_user) == 0:
    ...
    coupon_name = frappe.get_list(
        "Coupon Code", filters={"coupon_code": promocode}, ignore_permissions=True
    )[0].name
    ...
    base_plan = (
        coupon_code.base_plan
        if saas_user_doc.country == "India"
        else coupon_code.base_plan_international
    )

    if coupon_code.is_stackable and (
        saas_site.subscription or saas_site.discounted_users > 0
    ):
        limit_users = (
            saas_site.limit_for_users + coupon_code.limit_for_users
            if coupon_code.limit_for_users
            else saas_site.limit_for_users
        )
        discounted_users = (
            saas_site.discounted_users + coupon_code.discounted_users
            if coupon_code.discounted_users
            else saas_site.discounted_users
        )
        if (
            coupon_code.max_discounted_user_limit
            and discounted_users > coupon_code.max_discounted_user_limit
        ):
            throw(
                _(
                    "Maximum allowed stack user limit is "
                    + str(coupon_code.max_discounted_user_limit)
                )
            )
            return False

        limit_emails = (
            saas_site.limit_for_emails + int(coupon_code.limit_for_emails)
            if int(coupon_code.limit_for_emails)
            else saas_site.limit_for_emails
        )
        limit_space = (
            saas_site.limit_for_space + int(coupon_code.limit_for_space)
            if int(coupon_code.limit_for_space)
            else saas_site.limit_for_space
        )
        limit_email_group = (
            saas_site.limit_for_email_group + int(coupon_code.limit_for_email_group)
            if int(coupon_code.limit_for_email_group)
            else saas_site.limit_for_email_group
        )
    else:
        discounted_users = (
            coupon_code.discounted_users
        ...
        limit_users = (
            0
            if coupon_code.unlimited_users
            else (
                int(coupon_code.limit_for_users)
                if coupon_code.limit_for_users
                else saas_site.limit_for_users
    ...
    if coupon_code.no_expiry == 1:
        ...
        limit_expiry = (
            add_days(today(), int(coupon_code.expiry))
            if coupon_code.expiry > 0
            else saas_site.expiry
        )

    saas_site.customer = (
        coupon_code.customer
        if coupon_code.customer and not saas_site.customer
        else saas_site.customer
    )
    ...
    if saas_site.subscription:
        ...
        if (
            coupon_code.unlimited_users
            or (int(saas_site.limit_for_users) - int(saas_site.discounted_users) <= 0)
        ) and coupon_code.no_expiry:
    ...
    success_message = (
        coupon_code.success_message
        if coupon_code.success_message
        else _("Promocode has been Applied Successfully.")
    )
    frappe.db.commit()
...
def update_promocode_on_reference_docs(saas_user, promocode):
    ...
    saas_user.promocode = (
        promocode if not saas_user.promocode else saas_user.promocode + "," + promocode
    )
    saas_user.save(ignore_permissions=True)

    lead = frappe.get_list(
        "Lead", filters={"email_id": saas_user.email}, ignore_permissions=True
    )
    if len(lead) > 0:
...
def refund_promocode(promocode):
    ...
    if coupon_code.status == "Available" or coupon_code.status == "Sold":
        ...
        saas_user = frappe.get_list(
            "Saas User",
            filters={"linked_saas_site": site_name},
            ignore_permissions=True,
        )
        if len(saas_user) == 0:
            frappe.throw("Invalid Request")
        ...
        if coupon_code.is_stackable:
            ...
            discounted_users = (
                0 if coupon_code.discounted_users else saas_site.discounted_users
            )
            limit_emails = (
                saas_settings.default_limit_for_emails
                if int(coupon_code.limit_for_emails)
                else saas_site.limit_for_emails
            )
            limit_space = (
                saas_settings.default_limit_for_space
                if int(coupon_code.limit_for_space)
                else saas_site.limit_for_space
            )
            limit_email_group = (
                saas_settings.default_limit_for_email_group
                if int(coupon_code.limit_for_email_group)
                else saas_site.limit_for_email_group
            )
            limit_users = (
                saas_settings.default_limit_for_users
                if coupon_code.unlimited_users
                else saas_settings.default_limit_for_users
                if coupon_code.limit_for_users
                else saas_site.limit_for_users
            )  ## Applying Users count from promocode
        ...
        if coupon_code.no_expiry == 1:
            ...
            limit_expiry = (
                add_days(saas_site.expiry, -1 * int(coupon_code.expiry))
                if coupon_code.expiry > 0
                else saas_site.expiry
            )
...
def get_promocode_benefits(site_name):
    saas_user = frappe.get_doc(
        "Saas User", {"linked_saas_site": site_name}, ignore_permissions=True
    )
    if saas_user.promocode:
        promocode = saas_user.promocode.split(",")
        coupon_codes = frappe.get_list(
            "Coupon Code",
            filters=[["coupon_code", "in", promocode], ["status", "!=", "Refunded"]],
            fields=["name", "coupon_code", "status", "success_message", "no_expiry"],
            ignore_permissions=True,
        )
    else:
        coupon_codes = []
